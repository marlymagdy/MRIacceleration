...
class CartesianScampi(UcnnReco):
    ...
    def prep_data(self):
        ...
        if (self.data['full_kspace'] is not None) and (self.data['mask'] is not None):
            ...
            # Get magnitude and phase of a
            target_min = 0.0064
            target_max = 5659.5693
            a_mag = torch.abs(ksp_full)
            a_phase = torch.angle(ksp_full)

            # Scale magnitudes
            a_min, a_max = a_mag.min(), a_mag.max()
            a_scaled_mag = (a_mag - a_min) / (a_max - a_min)
            a_scaled_mag = a_scaled_mag * (target_max - target_min) + target_min

            # Reconstruct complex tensor with scaled magnitude and original phase
            ksp_full = a_scaled_mag * torch.exp(1j * a_phase)
